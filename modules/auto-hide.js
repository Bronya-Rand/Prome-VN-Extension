...
import { getGroupIndex, getRecentTalkingCharacters, isAutoHideSpritesEnabled, isUserSpriteEnabled } from "../utils.js";
...
export async function handleAutoHideSprites() {
    ...
    if (isAutoHideSpritesEnabled()) {
        const maxVisibleCharacters = extension_settings[extensionName].maxViewableCharacters;

        if (allSprites.length > maxVisibleCharacters) {
            const recentTalkingCharacters = getRecentTalkingCharacters(maxVisibleCharacters);
            ...
            allSprites.each((_, sprite) => {
                ...
                if (!recentTalkingCharacters.includes(spriteId.replace("expression-", ""))) {
